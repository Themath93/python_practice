...
# 마지막에 2개의 원소 추가하면 에러
# a.append(8,9) #TypeError: list.append() takes exactly one argument (2 given)
...
# 마지막에 원소 삽입
nums.insert(-1,"홍길동") #[1, 200, 2, 3, 4, '홍길동', 5] # 마지막 원소 앞에 삽입
print(nums)
print(len(nums))
nums.insert(len(nums),12.3) #[1, 200, 2, 3, 4, '홍길동', 5, 12.3] # 맨 뒤에 삽입
print(nums)

nums.insert(len(nums)-1, [10,20]) # 마지막 원소 앞에 하위 리스트 삽입
print(nums)

# remove() / pop() : 리스트 요소 제거
...
#     # 반복 횟수를 리스트 원소수만큼 설정했고 삭제 할 값이 더이상 없으면 에러가 발생
# print(n)
...
x = ['a','b','c','d']
y = x.pop() # x리스트이 마지막 요소 반환하고 그 요소를 리스트에서 삭제
print(y) #  d
print(x) #['a', 'b', 'c']
x.pop()
x.pop()
x.pop()
print(x) #[] pop 계속해서 더이상 요소가 없으면 빈 리스트가 남는다

# x.pop() #IndexError: pop from empty list
# print(x)

# pop(인덱스) : 인덱스 위치에 있는 요소 반환 하고 해당 요소 삭제
...
a = [1,2,3]
# a.append([4,5]) #[1, 2, 3, [4, 5]] 새로추가한 리스트가 하위 리스트로 추가
...
scores = [90,78,81,64,89]
scores.sort(reverse=True) #[90, 89, 81, 78, 64]
print(scores)

# reverse()
# 원소의 위치를 역순으로 변경(정렬은 하지않고 원소들의 순서만 바꾼다)
scores = [90,78,81,64,89]
scores.reverse() # [89, 64, 81, 78, 90]
print(scores)
...
# 대소문자 구별 없이 정렬
char = ['b','A','d','C']
char.sort(key=str.lower) #['A', 'b', 'C', 'd']
print(char)

# 대소문자 구별없이 내림차순 정렬
char = ['b','A','d','C']
char.sort(key=str.lower,reverse=True) #['d', 'C', 'b', 'A']
print(char)

# 문자열의 정렬 : 첫 문자를 기준으로 정렬
...
# index(찾을값) : 리스트 안에서 특정 원소를 찾아 원소의 위치값(인덱스) 반환
# 원소가 존재하지 않으면 에러
