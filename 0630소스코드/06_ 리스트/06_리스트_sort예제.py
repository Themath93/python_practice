...
# 입력받은 학생들의 점수를 높은 점수부터 출력되게 정렬 할것
# 학생수는 사용자에게 입력받아서 사용자가 입력한 학생 수만큼의 점수를 입력받을 것

# 학생들의 점수를 입력받아 저장할 빈 리스트 생성
scores = []
# 총점을 계산하기 위한 변수
sum_s = 0
...
# 평균은 총점 구하고 학생수로 나누어줌(학생 수는 num_s에 저장, scores리스트의 길이)
# 총점과 80점 이상 학생수는 리스트를 순회하면서 원소 하나씩 더하고 80점 이상 여부 판단

# 리스트내 점수합계와 80점 이상인 학생 수 계산
for s in scores :
    sum_s += s
    if s >= 80 :
        count += 1
...
print("총점 : %d" % sum_s)
print("평균 : %.2f" % avg) # 소숫점이하 2째자리까지 출력
