...
class Dict_Const :
    ...
    def __init__(self,file_name):
        self.file_name = file_name # 사전을 저장하기 위한 파일 명
        self.ek_dic = {} # 사전 구성 dict

    def const_dict(self):
        while True:  # 단 사전등록은 사용자가 원하는 만큼 등록이 가능하게 할 것이므로 무한루프 사용
            # 영어단어 입력 받기
            eng = input("\n영어 단어 입력 (종료는 'q') : ")
            ...
            elif eng not in self.ek_dic:  # 등록 되지 않은 단어
                # 단어 등록 처리 함수 호출
                self.input_dict(eng)
            else:
    ...
    def dict_search(self,eng):  # 단어가 사전에 있는 경우 호출
        print("%s 의 뜻은 %s 입니다. " % (eng, self.ek_dic[eng]))
